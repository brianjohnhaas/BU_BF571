...
sub main {
    ...
    open(my $fh, "$data_home/KO.listing") or die "Error, cannot find file: $data_home/KO.listing";
    while(<$fh>) {
        unless (/\w/) { next; }
        chomp;
        if (/^BF_main_(GSM\d+)_(TOP|LAST)_5min.m:KO = \[(.*)\];/) {
            my $GSM_name = $1;
            my $top_or_last = $2;
            my $KO_list = $3;
...
for i = 1:n_KO
    ...
    save('-v7.3', fullfile('.', 'data', 'output', ['BF_isgMCS_' GSM_name(1:end-4) '___TOP_OR_LAST___' '__KO_VAL___' num2str(timelimit/60) 'min.mat']));
end
